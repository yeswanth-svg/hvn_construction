                                        ...
                                        <td>
                                            <div class="form-button-action">
                                                ...
                                                <button class="btn btn-link btn-secondary show-button"
                                                    data-id="{{$account->id}}" title="Show" id="openShow">
                                                    <i class="fa fa-eye"></i>
                                                </button>

                                                <button class="btn btn-link ms-auto edit-button"
                                                    data-id="{{ $account->id }}" title="Edit" id="openEdit">
                                                    <i class="fa fa-edit"></i>
                                                </button>

                                                <form
                                                    action="{{ route('admin.company-accounts.destroy', $account->id) }}"
                                                    method="POST" class="delete-form" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-link btn-danger delete-btn"
                                                        data-url="{{ route('admin.company-accounts.destroy', $account->id) }}"
...
<script>
    $(document).ready(function () {
        ...
        $('#openCreate').click(function () {
            window.location.href = '{{ route('admin.company-accounts.create') }}';
        });

        // Handle editing a 
        $(document).on('click', '.edit-button', function () {
            var id = $(this).data('id'); // Get the team member ID from the data-id attribute
            var editUrl = '{{ route('admin.company-accounts.edit', ':id') }}'.replace(':id', id); // Replace :id with the actual ID
            window.location.href = editUrl; // Redirect to the edit page
        });
        // Handle showing team member details in modal
        $(document).on('click', '.show-button', function () {
            var id = $(this).data('id'); // Get the team member ID from the data-id attribute
            var showUrl = '{{ route('admin.company-accounts.show', ':id') }}'.replace(':id', id); // Replace :id with the actual ID
