...
@section('title', 'Plot Level Information')
@section('content')
<style>
    /* For smaller screens */
    .table-responsive {
                            ...
                            <table id="add-row" class="display table table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Developer Plots</th>
                                        <th>Land Owner Plots</th>
                                        ...
                                        <td>
                                            <div class="form-button-action">
                                                ...
                                                <button class="btn btn-link btn-secondary show-button"
                                                    data-id="{{$plot->id}}" title="Show" id="openShow">
                                                    <i class="fa fa-eye"></i>
                                                </button>

                                                <button class="btn btn-link ms-auto edit-button"
                                                    data-id="{{ $plot->id }}" title="Edit" id="openEdit">
                                                    <i class="fa fa-edit"></i>
                                                </button>

                                                <form
                                                    action="{{ route('admin.plot-level-information.destroy', $plot->id) }}"
                                                    method="POST" class="delete-form" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-link btn-danger delete-btn"
                                                        data-url="{{ route('admin.plot-level-information.destroy', $plot->id) }}"
...
<script>
    $(document).ready(function () {
        ...
        $('#openCreate').click(function () {
            window.location.href = '{{ route('admin.plot-level-information.create') }}';
        });

        // Handle editing a 
        $(document).on('click', '.edit-button', function () {
            var id = $(this).data('id'); // Get the team member ID from the data-id attribute
            var editUrl = '{{ route('admin.plot-level-information.edit', ':id') }}'.replace(':id', id); // Replace :id with the actual ID
        ...
        $(document).on('click', '.show-button', function () {
            var id = $(this).data('id'); // Get the team member ID from the data-id attribute
            var showUrl = '{{ route('admin.plot-level-information.show', ':id') }}'.replace(':id', id); // Replace :id with the actual ID
